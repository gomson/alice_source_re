  ...
  extends AbstractBodyPositionAnimation
{
  ...
  public class RuntimeTouchAnimation extends AbstractBodyPositionAnimation.RuntimeAbstractBodyPositionAnimation {
    public RuntimeTouchAnimation() { super(); }
    ...
    public void update(double t)
    {
      if (getPortion(t) <= 1.0D)
      {
        ...
        upperLimb.setOrientationRightNow(upperTargetQuat, (ReferenceFrame)upperLimb.getParent());
        

        if (lowerLimb != null) lowerLimb.setOrientationRightNow(lowerTargetQuat, (ReferenceFrame)lowerLimb.getParent());
      }
    }
    ...
    protected void setTargetQuaternions()
    {
      ...
      if (targetDistance < upperLimbLength + lowerLimbLength)
      {
        ...
        if (!Double.isNaN(limbAngle))
        {
          ...
          if ((limb.getLimbValue().equals(Limb.leftArm)) || (limb.getLimbValue().equals(Limb.rightArm))) {
            if (lowerLimb != null) {
              lowerLimb.turnRightNow(Direction.BACKWARD, limbAngle);
              upperLimb.turnRightNow(Direction.FORWARD, limbAngle / 2.0D);
            }
          }
          else if (lowerLimb != null) {
            lowerLimb.turnRightNow(Direction.FORWARD, limbAngle);
      ...
      lowerTargetQuat = null;
      if (lowerLimb != null) {
        lowerTargetQuat = lowerLimb.getOrientationAsQuaternion((ReferenceFrame)lowerLimb.getParent());
        if (Double.isNaN(limbAngle)) {
          lowerLimb.setTransformationRightNow(initialTrans, (ReferenceFrame)lowerLimb.getParent());
          lowerTargetQuat = lowerLimb.getOrientationAsQuaternion((ReferenceFrame)lowerLimb.getParent());
        }
      }
