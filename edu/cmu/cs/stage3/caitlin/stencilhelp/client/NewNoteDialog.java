...
class NewNoteDialog extends JWindow
{
  ...
  public NewNoteDialog(StencilManager paramStencilManager, TutorialEditor paramTutorialEditor) throws HeadlessException
  {
    ...
    localJPanel3.add(new javax.swing.JScrollPane(textArea), "Center");
    ...
    addMouseMotionListener(new MouseAdapter()
    {
      ...
      {
        setLocation(paramAnonymousMouseEvent.getLocationOnScreen());
        repaint();
      }
    });
    setLocation(300, 300);
  ...
  private JPanel createTypePanel()
  {
    ...
    noteTypesGroup = new ButtonGroup();
    noteTypeButtons = new JRadioButton[] { new JRadioButton("note"), new JRadioButton("reference"), new JRadioButton("hole") };
    noteTypeButtons[0].setSelected(true);
    for (JRadioButton localJRadioButton : noteTypeButtons)
    {
      noteTypesGroup.add(localJRadioButton);
      localJPanel.add(localJRadioButton);
    }
    noteTypeButtons[0].addActionListener(new ActionListener()
    {
      public void actionPerformed(ActionEvent paramAnonymousActionEvent)
      {
        hasNextCheckbox.setEnabled(true);
        autoAdvanceCheckbox.setEnabled(false);
      }
    });
    noteTypeButtons[1].addActionListener(new ActionListener()
    {
      public void actionPerformed(ActionEvent paramAnonymousActionEvent)
      {
        hasNextCheckbox.setEnabled(true);
    ...
    noteTypeButtons[2].addActionListener(new ActionListener()
    {
      public void actionPerformed(ActionEvent paramAnonymousActionEvent)
      {
        autoAdvanceCheckbox.setEnabled(true);
        hasNextCheckbox.setEnabled(false);
      }
    });
    return localJPanel;
  }
  ...
  {
    JPanel localJPanel = new JPanel(new GridLayout(3, 1));
    hasNextCheckbox = new JCheckBox("has next");
    autoAdvanceCheckbox = new JCheckBox("auto advance");
    hasNextCheckbox.setSelected(true);
    autoAdvanceCheckbox.setSelected(false);
    autoAdvanceCheckbox.setEnabled(false);
    localJPanel.add(hasNextCheckbox);
    localJPanel.add(new JLabel(" "));
    localJPanel.add(autoAdvanceCheckbox);
    return localJPanel;
  }
  ...
  private JPanel createOkCancelPanel()
  {
    JButton localJButton1 = makeButton("OK", new AbstractAction()
    {
      ...
      {
        Note localNote = null;
        if (NewNoteDialog.getSelection(noteTypesGroup) == noteTypeButtons[0]) {
          localNote = stencilManager.createNewNote(NewNoteDialog.this.getPosition(), hasNextCheckbox.isSelected());
        } else if (NewNoteDialog.getSelection(noteTypesGroup) == noteTypeButtons[1]) {
          localNote = stencilManager.createNewFrame(NewNoteDialog.this.getPosition(), hasNextCheckbox.isSelected());
        } else if (NewNoteDialog.getSelection(noteTypesGroup) == noteTypeButtons[2])
          localNote = stencilManager.createNewHole(NewNoteDialog.this.getPosition(), autoAdvanceCheckbox.isSelected());
        localNote.addText(textArea.getText(), null);
        dispose();
      }
    });
