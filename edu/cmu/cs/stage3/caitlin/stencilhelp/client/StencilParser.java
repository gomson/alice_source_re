...
public class StencilParser
{
  ...
  protected void loadNote(Node node, StencilManager.Stencil newStencil, NavigationBar navBar) {
    ...
    if ((hasNextNode != null) && 
      (hasNextNode.getNodeValue().equals("true"))) {
      hasNext = true;
    }
    ...
    if (objectType.getNodeValue().equals("hole")) {
      ...
      if ((autoAdvanceNode != null) && 
        (autoAdvanceNode.getNodeValue().equals("true"))) {
        autoAdvance = true;
      }
  ...
  public Vector parseFile(java.io.File fileToLoad) {
    ...
    } catch (java.io.IOException ioe) { Document document;
  ...
  public StencilManager.Stencil getErrorStencil()
  {
    ...
    Point p = new Point((int)((float)stencilApp.getScreenSize().getWidth() * 0.292D), (int)((float)stencilApp.getScreenSize().getHeight() * 0.448D));
